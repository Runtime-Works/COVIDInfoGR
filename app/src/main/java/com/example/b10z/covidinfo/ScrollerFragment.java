...
public class ScrollerFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        for (int i = 0; i<Timelinelist.get(0).length; i++ ) {
            if (i > 2) {
                TimelineEntries.add(new BarEntry((float)(i-2),Float.valueOf(Timelinelist.get(Timelinelist.size() - 1)[i])));
            }
        }

        BarDataSet set = new BarDataSet(TimelineEntries, "Total Cases/Time");
